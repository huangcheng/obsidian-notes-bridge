...
import { ImaProviderConfig } from "./providers/ima/types";
...
import { FLOMO_NAME, WPS_NAME, WEKNORA_NAME, YOUDAO_NAME, YINXIANG_NAME, IMA_NAME } from "./ui/brand-names";
...
export default class AdvancedImportExportPlugin extends Plugin {
	...
	async onload(): Promise<void> {
		...
		this.addCommand({
			id: "export-active-to-ima",
			name: t("commands.sendToIma"),
			checkCallback: (checking) => {
				if (this.listImaConfigs().length === 0) return false;
				const sel = selectionFromActiveEditor(this.app);
				if (!sel || sel.notes.length === 0) return false;
				if (!checking) void this.pickImaAndExport(sel.notes);
				return true;
			},
		});
 ...
 new Notice(t("notices.bearNotConfigured"));
			...
			if (config.kind === "bear") {
				...
				menu.addItem((item) =>
					item
						...
						.onClick(async () => {
							try {
								await this.exportToProvider(config as WpsProviderConfig | YoudaoProviderConfig | FlomoProviderConfig | YinxiangProviderConfig | WeknoraProviderConfig | ImaProviderConfig, files);
							} catch (err) {
	...
	private listImaConfigs(): ImaProviderConfig[] {
		return this.settings.providers.filter(
			(p): p is ImaProviderConfig => p.kind === "ima" && p.enabled !== false,
		);
	}

	private async pickImaAndExport(files: TFile[]): Promise<void> {
		const configs = this.listImaConfigs();
		if (configs.length === 0) {
			new Notice(t("notices.noProviderConfigured", { provider: IMA_NAME }));
			return;
		}
		const target = configs.length === 1 ? configs[0]! : await this.pickProviderConfig(configs, t("notices.selectProvider", { provider: IMA_NAME }));
		if (!target) return;
		await this.exportToProvider(target, files);
	}

	private async exportToProvider(
		config: WpsProviderConfig | YoudaoProviderConfig | FlomoProviderConfig | YinxiangProviderConfig | WeknoraProviderConfig | ImaProviderConfig,
		files: TFile[],
	): Promise<void> {
		const provider = this.registry.get(config.id);
		if (!provider) {
